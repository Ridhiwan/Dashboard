...
# ---- UPLOAD FILE ----
def uploaded(key):
    uploaded_doc = st.sidebar.file_uploader(label="Upload your file(CSV or Excel)",
...
def analysis():
    ...
    if get_data_from_excel() is not None:
        datatable = get_data_from_excel()

        # ----SIDEBAR----
        st.sidebar.header("Filter:")
        city = st.sidebar.multiselect(
            "Select City:",
            options=datatable["City"].unique(),
            default=datatable["City"].unique(),
        )

        customer_type = st.sidebar.multiselect(
            "Select Customer Type:",
            options=datatable["Customer_type"].unique(),
            default=datatable["Customer_type"].unique(),
        )

        gender = st.sidebar.multiselect(
            "Select Gender:",
            options=datatable["Gender"].unique(),
            default=datatable["Gender"].unique(),
        )

        df_selection = datatable.query(
